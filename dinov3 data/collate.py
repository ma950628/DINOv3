...
def collate_data_and_cast(
    ...
    # ===== MODIFICATION: iBOT disabled, skip mask generation and return dummy tensors =====
    # # Original mask generation code:
    ...
    # Dummy tensors to maintain expected keys in the output dict
    collated_masks = torch.zeros((B, N), dtype=torch.bool)
    mask_indices_list = torch.tensor([], dtype=torch.long)
    masks_weight = torch.tensor([], dtype=torch.float32)
    upperbound = 0
    n_masked_patches = torch.tensor(0, dtype=torch.long)
    # ===== END MODIFICATION =====
